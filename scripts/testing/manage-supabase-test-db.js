...
function assertSupabaseCliAvailable() {
  try {
    ...
    console.error('❌ Supabase CLI is required for disposable test databases.');
...
function ensureEnvFile(env) {
  ...
  console.log(`✅ Wrote Supabase environment variables to ${path.relative(PROJECT_ROOT, ENV_FILE)}`);
  return updates;
}
...
function runTestProvisioning() {
  ...
  console.log('\n🧹 Cleaning any existing test data (local instance)...');
  run('npm', ['run', 'test:reset'], { stdio: 'inherit' });

  console.log('\n🌱 Seeding fixtures via complete-test-setup.js...');
...
try {
  switch (action) {
    case 'start': {
      assertSupabaseCliAvailable();
      console.log('🚀 Starting local Supabase (disposable test instance)...');
