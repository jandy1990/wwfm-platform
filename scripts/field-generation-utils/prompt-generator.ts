...
import { getContextSources, getDropdownSource } from './category-config'
...
export function generateContextAwarePrompt(
  ...
  // Get the correct dropdown options for this field/category
  ...
  // Get context sources for this category
...
/**
 * Get appropriate source attribution for a category
 */
...
/**
 * Generate field-specific contextual hints
 */
...
/**
 * Create enhanced prompt with field-specific context
 */
...
/**
 * Generate fallback prompt if initial generation fails
 */
...
/**
 * Validate prompt generation inputs
 */
export function validatePromptInputs(
  ...
  if (!solutionName || typeof solutionName !== 'string') {
    errors.push('Invalid solution name')
  }
  ...
  if (!goalTitle || typeof goalTitle !== 'string') {
    errors.push('Invalid goal title')
  }

  // Check if dropdown source exists
  try {
    const dropdownSource = getDropdownSource(fieldName, category)
    const dropdownOptions = DROPDOWN_OPTIONS[dropdownSource]
    if (!dropdownOptions || dropdownOptions.length === 0) {
      errors.push(`No dropdown options available for ${fieldName} in ${category}`)
    }
  } catch (error) {
    errors.push(`Dropdown configuration error: ${error.message}`)
  }
