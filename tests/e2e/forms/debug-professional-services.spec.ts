...
test('Debug SessionForm professional_services', async ({ page }) => {
  console.log('=== Starting professional_services debug test ===');
  ...
  // Type the test solution for professional_services
  await page.fill('#solution-name', 'Career Coach (Test)');
  console.log('Typed solution name: Career Coach (Test)');
  ...
  if (pageContent?.includes('Choose a category')) {
    console.log('Category picker appeared - selecting professional_services');
  ...
  if (formContent?.includes('How well it worked')) {
    ...
    const specialtyLabel = await page.locator('label:has-text("Type of service")').isVisible();
    console.log(`\nSpecialty field visible: ${specialtyLabel}`);
    ...
    try {
      ...
      if (finalContent?.includes('Thank you')) {
        ...
        console.log('📍 Currently on Step 2 - Challenges (correct for professional_services)');
      ...
      if (isDisabled) {
        ...
        // Check specialty field specifically
        const specialtyCombobox = await page.locator('button[role="combobox"]:has-text("Select service type")').isVisible().catch(() => false);
        if (specialtyCombobox) {
          console.log('  ⚠️ Specialty field not filled!');
        }
      }
    }
  } else {
