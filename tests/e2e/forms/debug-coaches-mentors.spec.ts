...
test('Debug SessionForm coaches_mentors', async ({ page }) => {
  console.log('=== Starting coaches_mentors debug test ===');
  ...
  // Type the test solution for coaches_mentors
  await page.fill('#solution-name', 'Life Coach (Test)');
  console.log('Typed solution name: Life Coach (Test)');
  ...
  if (pageContent?.includes('Choose a category')) {
    console.log('Category picker appeared - selecting coaches_mentors');
