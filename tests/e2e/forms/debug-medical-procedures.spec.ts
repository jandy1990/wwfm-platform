...
test('Debug SessionForm medical_procedures', async ({ page }) => {
  console.log('=== Starting medical_procedures debug test ===');
  ...
  // Type the test solution for medical_procedures
  await page.fill('#solution-name', 'Endoscopy (Test)');
  console.log('Typed solution name: Endoscopy (Test)');
  ...
  if (pageContent?.includes('Choose a category')) {
    console.log('Category picker appeared - selecting medical_procedures');
    
    // Click the category
    await page.click('button:has-text("People you see")');
    await page.waitForTimeout(500);
    await page.click('button:has-text("Medical Procedures")');
  ...
  if (formContent?.includes('How well it worked')) {
    ...
    // Check for wait time field
    const waitTimeLabel = await page.locator('label:has-text("Wait time")').isVisible();
    console.log(`\nWait time field visible: ${waitTimeLabel}`);
    
    // Check cost type options
    const totalCostOption = await page.locator('label:has-text("Total cost")').isVisible();
    console.log(`Total cost option visible: ${totalCostOption}`);
    
    // Use the fillSessionForm function
    try {
      await fillSessionForm(page, 'medical_procedures');
      ...
      if (finalContent?.includes('Thank you')) {
        ...
        console.log('📍 Currently on Step 2 - Side effects (correct for medical_procedures)');
      ...
      if (isDisabled) {
        ...
        // Check wait time field specifically
        const waitTimeCombobox = await page.locator('button[role="combobox"]:has-text("Time to get appointment")').isVisible().catch(() => false);
        if (waitTimeCombobox) {
          console.log('  ⚠️ Wait time field not filled!');
        }
      }
    }
  } else {
