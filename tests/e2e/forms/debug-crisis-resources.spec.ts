...
test('Debug SessionForm crisis_resources', async ({ page }) => {
  console.log('=== Starting crisis_resources debug test ===');
  ...
  // Type the test solution for crisis_resources
  await page.fill('#solution-name', 'Suicide Hotline (Test)');
  console.log('Typed solution name: Suicide Hotline (Test)');
  ...
  if (pageContent?.includes('Choose a category')) {
    console.log('Category picker appeared - selecting crisis_resources');
  ...
  if (formContent?.includes('How well it worked')) {
    ...
    // Check for response time field
    const responseTimeLabel = await page.locator('label:has-text("Response time")').isVisible();
    console.log(`\nResponse time field visible: ${responseTimeLabel}`);
    
    // Check for session frequency (should NOT be visible)
    const sessionFrequencyLabel = await page.locator('label:has-text("Session frequency")').isVisible();
    console.log(`Session frequency field visible: ${sessionFrequencyLabel} (should be false)`);
    
    // Check for cost type radio buttons (should NOT be visible)
    const costTypeRadios = await page.locator('input[name="costType"]').count();
    console.log(`Cost type radio buttons: ${costTypeRadios} (should be 0)`);
    ...
    try {
      ...
      } else if (finalContent?.includes('Any challenges')) {
        console.log('📍 Currently on Step 2 - Challenges (correct for crisis_resources)');
        
        // Check if challenges are being handled
        const noneCheckbox = await page.locator('label:has-text("None")').isVisible();
        if (noneCheckbox) {
          console.log('✅ "None" challenge option is visible');
        }
      } else {
      ...
      if (isDisabled) {
        ...
        // Check response time field specifically
        const responseTimeCombobox = await page.locator('button[role="combobox"]:has-text("How quickly did they respond?")').isVisible().catch(() => false);
        if (responseTimeCombobox) {
          console.log('  ⚠️ Response time field not filled!');
        }
      }
    }
  } else {
